...
void BlockDown(int sig){
	int moveFlag;
	moveFlag = CheckToMove(field, nextBlock[0], blockRotate, blockY+1, blockX);
	if(moveFlag){
		blockY++;
...
int DeleteLine(char f[HEIGHT][WIDTH]){
	...
	for(y=0; y < HEIGHT; y++){
		...
		if(isfull == WIDTH){
			...
			for(temp_x=0; temp_x < WIDTH; temp_x++) f[temp_y][temp_x] = '.';
		}
	}

	return (line_removed * line_removed) * 100;	
